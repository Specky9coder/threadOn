...
class Share_ItemList extends StatefulWidget {
  final Shell_Product_Model item;

  const Share_ItemList({@required this.item});

  @override
  State<StatefulWidget> createState() => new shareitemlist(item);
// TODO: implement createState
}

class shareitemlist extends State<Share_ItemList> {
  final Shell_Product_Model item;
  ...
  Future _openAddUserDialog(String id) async {
    ...
    if (eventsQuery.documents.isEmpty) {
      ...
      eventsQuery.documents.forEach((doc) async {
        ...
        shareList.add(Share(doc['share_id'],doc['user_id'], doc['date'],doc['share_list_name'],doc['share_product_id']));
...
//                    icon: Icon(Icons.favorite_border, color: Colors.black,),
  ...
  handleSubmit() async {
    ...
    noteSub?.cancel();
    db.createShareList('', user_id, DateTime.now(), submit,share_product_id)
        .then((_) {
      setState(() {
        _isInAsyncCall = false;
      });
  ...
  //  share_product_id.removeWhere((item1) => item1.length == item.product_id);
   // share_product_id.remove(item.product_id);
    ...
    if (eventsQuery.documents.isEmpty) {
      ...
      eventsQuery.documents.forEach((doc) async {
          ...
          share_product_id = new List.from(doc['share_product_id']);
          ...
          db1.collection("share_list")
            ...
            setState(() {
              ...
              showInSnackBar('product successfully delete to share list.');
                    ...
                    child: Container(
                      ...
                      child: IconButton(
                        icon: new Icon(
                          ...
                          /*  var db1 = Firestore.instance;
                            db1.collection("share_list").document(Share_id)
                                .delete()
                                .then((val) {
                             Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Share_List_Screen(title: '',)));
                              print("sucess");
                              setState(() {
                                _isInAsyncCall = false;
                              });
                      ...
                      if (Flag1 == "0") {
                        if (productId == item.product_id) {
                          ...
                          Navigator.pushReplacement(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => Compare_screen(item: item)));
                        }
                      } else {
                        Navigator.push(
                          context,
